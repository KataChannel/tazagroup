...
const BACKUP_ROOT_DIR = './backups/tazagroup';
...
async function getTablesToRestore(backupFolder: string): Promise<string[]> {
  ...
  // Skip audit_logs (too large - 410MB)
  const restorationOrder = [
    ...
    // Pages
    'pages', 'page_blocks',
    // Skip: audit_logs (410MB - too large for restore)
    // Roles & Security
    'rbac_roles', 'rbac_permissions', 'rbac_role_permissions',
    // Menus & Settings
    'menus', 'website_settings',
    // Courses
    'course_categories', 'courses', 'course_modules', 'lessons', 
    'enrollments', 'lesson_progress', 'quizzes', 'questions', 'answers',
    // Reviews and other
    'reviews', 'carts',
...
async function main(): Promise<void> {
  ...
  try {
    // Use specific backup folder instead of latest
    const backupFolder = process.env.BACKUP_FOLDER || '20251125_012515';
