...
describe('EnrollmentsService', () => {
  ...
  describe('enroll', () => {
    ...
    it('should throw NotFoundException if course not found', async () => {
      ...
      await expect(service.enroll('user-1', 'non-existent')).rejects.toThrow(
    ...
    it('should throw BadRequestException if course is not published', async () => {
      ...
      await expect(service.enroll('user-1', 'course-1')).rejects.toThrow(
        new BadRequestException('Cannot enroll in unpublished course'),
      );
    });
  ...
  describe('updateProgress', () => {
    ...
    it('should throw NotFoundException if enrollment not found', async () => {
      ...
      await expect(service.updateProgress('user-1', 'course-1')).rejects.toThrow(
  ...
  describe('dropCourse', () => {
    ...
    it('should throw NotFoundException if enrollment not found', async () => {
      ...
      await expect(service.dropCourse('user-1', 'course-1')).rejects.toThrow(
        new NotFoundException('Enrollment not found'),
      );
    });
    ...
    it('should throw BadRequestException if trying to drop completed course', async () => {
      ...
      await expect(service.dropCourse('user-1', 'course-1')).rejects.toThrow(
        new BadRequestException('Cannot drop a completed course'),
      );
    });
  });
  ...
  describe('getCourseEnrollments', () => {
    ...
    it('should throw ForbiddenException if user is not course instructor', async () => {
      ...
      await expect(
        service.getCourseEnrollments('course-1', 'different-user'),
      ).rejects.toThrow(
        new ForbiddenException('You do not have permission to view these enrollments'),
      );
    });
  });
  ...
  describe('markLessonComplete', () => {
    it('should mark a lesson as complete', async () => {
      ...
      // Mock for updateEnrollmentProgress
    ...
    it('should throw NotFoundException if enrollment not found', async () => {
      ...
      await expect(
        service.markLessonComplete('user-1', 'enrollment-1', 'lesson-1'),
    ...
    it('should throw ForbiddenException if enrollment does not belong to user', async () => {
      ...
      await expect(
        ...
        new ForbiddenException('Not authorized to update this enrollment'),
      );
    });
