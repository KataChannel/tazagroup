...
function CampaignCard({ campaign, t }: { campaign: Campaign; t: any }) {
    ...
    <Card className="group hover:shadow-lg transition-all duration-200">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-xs">
          <div>
            <p className="text-muted-foreground">{t('campaigns.commission')}</p>
            <p className="font-semibold text-green-600">{campaign.commission}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{t('campaigns.minPayout')}</p>
            <p className="font-medium">{campaign.minPayout}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{t('campaigns.cookie')}</p>
            <p className="font-medium">{campaign.cookieDuration}</p>
          </div>
          <div>
            <p className="text-muted-foreground">{t('campaigns.status')}</p>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="space-y-8">
          ...
          <TabsList className="grid w-full grid-cols-3 lg:grid-cols-6">
            {categories.map((category) => (
              <TabsTrigger key={category} value={category} className="text-xs">
                {category === "all" ? t('campaigns.all') : category}
              </TabsTrigger>
            ))}
          </TabsList>
        </Tabs>
        ...
        <div className="text-sm text-muted-foreground">
          {t('campaigns.showing', { count: filteredCampaigns.length })}
        </div>
