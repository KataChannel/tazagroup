...
${BLUE}SUPPORT:${NC}
    📧 Email: deploy-support@tazav1.com
    📚 Docs:  https://docs.tazav1.com/deployment
    💬 Chat:  https://discord.gg/tazav1
...
setup_wizard() {
    log_header "🚀 Tazav1 Setup Wizard"
    
    echo -e "${BLUE}Welcome to Tazav1 deployment setup!${NC}"
...
deploy_application() {
    ...
    log_header "🚀 Deploying Tazav1 Application"
...
create_backup() {
    ...
    local backup_name="tazav1_backup_${env}_${timestamp}"
    ...
    case $backup_type in
        "database")
            ...
            ssh "$user@$server" "
                docker-compose exec -T postgres pg_dump -U postgres tazav1 | gzip > ${backup_name}.sql.gz
        ...
        "full")
            ...
            ssh "$user@$server" "
                docker-compose exec -T postgres pg_dump -U postgres tazav1 | gzip > ${backup_name}_db.sql.gz
...
health_check() {
    ...
    if ssh "$user@$server" "
        ...
        docker-compose exec -T postgres psql -U postgres -d tazav1 -c 'SELECT 1;' > /dev/null 2>&1 && echo 'Database: OK' || echo 'Database: ERROR'
