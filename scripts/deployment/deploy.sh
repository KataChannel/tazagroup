...
# Katadev Master Deployment Script
...
${BLUE}SUPPORT:${NC}
    📧 Email: deploy-support@katadev.com
    📚 Docs:  https://docs.katadev.com/deployment
    💬 Chat:  https://discord.gg/katadev
...
setup_wizard() {
    log_header "🚀 Katadev Setup Wizard"
    
    echo -e "${BLUE}Welcome to Katadev deployment setup!${NC}"
...
deploy_application() {
    ...
    log_header "🚀 Deploying Katadev Application"
...
create_backup() {
    ...
    local backup_name="katadev_backup_${env}_${timestamp}"
    ...
    case $backup_type in
        "database")
            ...
            ssh "$user@$server" "
                docker-compose exec -T postgres pg_dump -U postgres katadev | gzip > ${backup_name}.sql.gz
        ...
        "full")
            ...
            ssh "$user@$server" "
                docker-compose exec -T postgres pg_dump -U postgres katadev | gzip > ${backup_name}_db.sql.gz
...
health_check() {
    ...
    if ssh "$user@$server" "
        ...
        docker-compose exec -T postgres psql -U postgres -d katadev -c 'SELECT 1;' > /dev/null 2>&1 && echo 'Database: OK' || echo 'Database: ERROR'
