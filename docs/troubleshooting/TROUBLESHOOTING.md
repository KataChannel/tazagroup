...
1. **Check Database Performance**
   ...
   docker-compose exec postgres psql -U postgres -d katadev
   ...
   # Check database size
   SELECT pg_database_size('katadev');
   ```
...
1. **Check Database Status**
   ...
   # Test connection
   docker-compose exec postgres psql -U postgres -d katadev -c "SELECT 1;"
   ```
...
2. **Backup Database Before Migration**
   ```bash
   # Create backup
   docker-compose exec postgres pg_dump -U postgres katadev > backup.sql
...
2. **Check Docker Network**
   ...
   docker network inspect katadev_default
...
docker-compose exec postgres psql -U postgres -d katadev -c "SELECT 1;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "Database: OK"
else
    echo "Database: ERROR"
...
echo "=== Katadev Performance Monitor ==="
...
docker-compose exec postgres psql -U postgres -d katadev -c "
SELECT 
    calls,
    mean_time,
    query
...
**Community Resources:**
- Discord: https://discord.gg/katadev
- GitHub Issues: https://github.com/katadev/issues
- Stack Overflow: Tag `katadev`
