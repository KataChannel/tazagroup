# ⚡ Tazav1 Quick Start Guide

Hướng dẫn bắt đầu nhanh cho Tazav1 - Deploy trong 5 phút!
...
### Xem logs
```bash
ssh root@YOUR_SERVER_IP 'cd /opt/tazav1 && docker compose logs -f'
```

### Credentials
```bash
# Xem passwords được tạo tự động
ssh root@YOUR_SERVER_IP 'cat /opt/tazav1/.env.prod'
```
...
```bash
# Restart services
ssh root@SERVER_IP 'cd /opt/tazav1 && docker compose restart'
...
```bash
# Xem logs chi tiết
ssh root@SERVER_IP 'cd /opt/tazav1 && docker compose logs'

# Rebuild containers
ssh root@SERVER_IP 'cd /opt/tazav1 && docker compose down && docker compose up -d --build'
```
...
**🎉 Congratulations! Tazav1 is now running!**  
└── config               # SSH configuration

# SSH Access Commands:
ssh -i ~/.ssh/tazav1-deploy root@116.118.85.41
# OR using the auto-generated alias:
ssh tazav1-116.118.85.41
```
...
3. ✅ Install and configure Tazav1
4. ✅ Show access URLs
...
| `deploy-remote.sh` | 🐳 Tazav1 deployment (enhanced) | `./deploy-remote.sh --simple 116.118.85.41 116.118.85.41` |
