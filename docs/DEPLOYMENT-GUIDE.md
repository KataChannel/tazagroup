# 🚀 Tazav1 Deployment Guide

Hướng dẫn triển khai toàn diện cho dự án Tazav1 với các công cụ tự động hóa.
...
Tazav1 cung cấp 2 script chính để tự động hóa quy trình phát triển và triển khai:
...
### 1. Clone Repository
```bash
git clone https://github.com/your-org/Tazav1.git
cd Tazav1
```
...
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose ps'

# Xem logs
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose logs'
...
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose restart'

# Restart service cụ thể
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose restart api'
```
...
# Update chỉ API
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose build api && docker compose up -d api'
```
...
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose down'
```
...
```bash
# Tất cả services
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose logs -f'

# Chỉ API
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose logs -f api'

# Last 100 lines
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose logs --tail=100'
```
...
```bash
# Xem chi tiết lỗi
ssh -i ~/.ssh/id_rsa root@SERVER_IP 'cd /opt/tazav1 && docker compose build --no-cache'
